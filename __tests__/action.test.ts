...
describe('markdown-list-linter action', () => {
    ...
    describe.each([true, false])('given failOnError is set to %s',  (failOnErrorResponse: boolean) => {
        it('should function as expected with a valid file', async () => {
            ...
            expect(setOutputMock).toBeCalledWith('formattedMessage', 'Summary: No errors')
        ...
        it('should function as expected with a valid file that needs to be reordered', async () => {
            const errorObject = [{ type: 'Headings', details: [['abc']], message: 'Needs fixes in alphabetical order'}]
            ...
            expect(setOutputMock).toBeCalledWith('formattedMessage', 'Summary: Needs fixes')
        ...
        it('should handle exception when one occurs in the main method', async () => {
            ...
            getBooleanInputMock.mockReturnValueOnce(failOnErrorResponse)
            lintMarkdownListMock.mockImplementationOnce(() => { throw new Error('Sample error')});
